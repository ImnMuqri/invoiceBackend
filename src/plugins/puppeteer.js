  ...
  fastify.decorate("generatePDF", async (url) => {
    ...
    try {
      ...
      /* Wait for images to finish decoding.
         This is here because of the line above: networkidle0 used to wait for
         every subresource as a side effect, and domcontentloaded fires before
         images have loaded at all. Without this the swap saves 500ms and prints
         the document with its logos missing — both the "Generated by" mark and,
         worse, the sender's own uploaded letterhead.

         Waiting on the actual images rather than on network silence is also
         more precise: it does not care about analytics beacons or anything else
         that happens to still be in flight. */
      await Promise.race([
        page.evaluate(() =>
          Promise.all(
              ...
              .map(
                (img) =>
                  new Promise((resolve) => {
                    ...
                    /* Resolve on error too. A broken logo should cost the
                       document its logo, not the whole PDF. */
